...
def test_company_add_http_returns_structured_409_on_conflict(monkeypatch) -> None:
    ...
    fn = cast(modal.Function, modal_companies.attio_company_add_http)  # type: ignore
    response = fn.local(
...
def test_note_update_http_infers_401_from_error_message(monkeypatch) -> None:
    ...
    fn = cast(modal.Function, modal_notes.attio_note_update_http)  # type: ignore
    response = fn.local(
...
def test_person_add_http_returns_non_2xx_when_envelope_failed(monkeypatch) -> None:
    ...
    fn = cast(modal.Function, modal_people.attio_person_add_http)  # type: ignore
    response = fn.local(
